$(document).ready(function(){
  ...
  window.addEventListener('message', function(event) {
      ...
      } else if (event.data.action == 'wrong') {
		$('#code h1').text('');
		code = '';
	  }
  });
  ...
  $("#b1").click(function() {
	  var x = 1
	  $.post('http://jsfour-atm/number', JSON.stringify({number : x}));
  });
  $("#b2").click(function() {
	  var x = 2
	  $.post('http://jsfour-atm/number', JSON.stringify({number : x}));
  ...
  $('#wrong').click(function(){
    ...
    $('#code h1').text('');
    code = '';
	$.post('http://jsfour-atm/cancelcode', JSON.stringify({}));
  });

  $('#done').click(function(){
    clickSound.play();
    if (code.length == 4) {
	  $.post('http://jsfour-atm/entercode', JSON.stringify({}));
